...
public class UberOger extends CreatureAtribute {
    ...
    public Creature attack(Creature victim) {
        final int victimHealthBefore = victim.getHealth();
        if (isAlive() && !getName().equals(victim.getName())) {
            while (victim.getHealth() > 1) {
                victim.hurt();
            }
        }
        if (victimHealthBefore > victim.getHealth()) {
            while (getHealth() < getMaxHealth()) {
                heal();
            }
        }
    ...
    public boolean equals(Object obj) {
        if (!super.equals(obj)) 
            return false;
        final UberOger other = (UberOger)obj;
